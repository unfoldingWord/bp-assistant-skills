#!/usr/bin/env python3
"""
fetch_hebrew_bible.py - Fetch Hebrew Bible (UHB) from Door43

Fetches all 39 OT books from git.door43.org/unfoldingWord/hbo_uhb
and saves them to data/hebrew_bible/ with date headers for caching.

Usage:
    python fetch_hebrew_bible.py                    # Fetch all books
    python fetch_hebrew_bible.py --books GEN EXO   # Fetch specific books
    python fetch_hebrew_bible.py --force           # Force re-fetch all
    python fetch_hebrew_bible.py --list            # List available books
"""
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "data", "hebrew_bible")
...
def fetch_book(book, output_dir, force=False):
    """Fetch a single book's USFM from Door43 UHB."""
    book = book.upper()
    ...
    # Fetch from Door43 UHB repo
    url = f"https://git.door43.org/unfoldingWord/hbo_uhb/raw/branch/master/{filename}"
...
def main():
    parser = argparse.ArgumentParser(
        description='Fetch Hebrew Bible (UHB) from Door43',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s                    # Fetch all 39 OT books
  %(prog)s --books GEN EXO    # Fetch specific books
  %(prog)s --force            # Force re-fetch all books
  %(prog)s --list             # List all OT books
"""
    )
    ...
    print(f"Fetching {len(books)} book(s) to {args.output_dir}")
    print()
