#!/usr/bin/env python3
"""
fetch_all_ult.py - Batch fetch ULT USFM files from Door43

Fetches published ULT books from git.door43.org/unfoldingWord/en_ult
and saves them to data/published_ult/ with date headers for caching.

Usage:
    python fetch_all_ult.py                    # Fetch all v88 published books
    python fetch_all_ult.py --books GEN EXO    # Fetch specific books
    python fetch_all_ult.py --force            # Force re-fetch all
    python fetch_all_ult.py --list             # List available books
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "data", "published_ult")

# v88 published OT books (from the plan)
# Note: NAM is Nahum (Door43 abbreviation differs from some conventions)
V88_PUBLISHED_BOOKS = [
...
def fetch_book(book, output_dir, force=False):
    ...
    # Fetch from Door43
    url = f"https://git.door43.org/unfoldingWord/en_ult/raw/branch/master/{filename}"
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch fetch ULT USFM files from Door43',
