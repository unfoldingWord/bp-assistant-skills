...
- **Active verbs with no explicit subject where English passive would be the natural rendering**: supply the implied subject in brackets to preserve active voice. Use a contextually appropriate noun — `{people}`, `{enemies}`, `{he}`, etc. — so an active Hebrew verb renders as `{people} forgot`, not "is forgotten." When the implied subject is genuinely unclear, try `{someone}` first before resorting to passive. (Verse-level rulings: `data/quick-ref/ult_decisions.csv`.)
